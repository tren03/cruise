...
import { fuseOptions, bookmarkFuseOptions, fuseColorOptions } from './fuseOpts'
import { availableColors, updateThemeColor, setThemeColor } from './colors'
import {
    searchInput,
    resultsContainer,
    mainContainer,
...
async function getTabs() {
    try {
        ...
        data.tabs.forEach((tab, _) => {
            if (tab.windowId == curwindowid) {
                //isCurWindow -> global - defined in index.js
                isCurWindow = true
            } else {
                isCurWindow = false
            }
      ...
      console.log(tab);
        ...
        fuseTab = new Fuse(data.tabs, fuseOptions)
        if (currentMode === 'T') {
            renderDropDown(data.tabs, true) // Render all tabs initially
        }
    } catch (error) {
...
async function getHistory() {
    try {
        ...
        fuseHistory = new Fuse(data.history, fuseOptions)
        if (currentMode === 'H') {
...
async function getTopSites() {
    try {
        ...
        fuseTopSites = new Fuse(data.topsites, fuseOptions)
        if (currentMode === 'F') {
...
async function getBookmarks() {
    try {
        ...
        fuseBookmark = new Fuse(data.bookmarks, bookmarkFuseOptions)
        if (currentMode === 'B') {
...
searchInput.addEventListener('input', (e) => {
    ...
    if (query === '') {
        ...
        if (currentMode === 'H') {
            renderDropDown(data.history, true)
        }
